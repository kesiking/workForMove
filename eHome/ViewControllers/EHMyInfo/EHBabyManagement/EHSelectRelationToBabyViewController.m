...
- (void)viewDidLoad {
    ...
    [self setupCurrentRelation:self.currentRelation];
...
- (void)setupCustomTextfield
{
    ...
    _customRelationTextFiled.text = self.currentRelation;
...
- (void)relationImageViewClick:(UITapGestureRecognizer*)sender
{
    ...
    _selectedRelation = _relationArray[clickImageView.tag - 1001];
    _customRelationTextFiled.text = _selectedRelation;
...
- (IBAction)confirmClicked:(id)sender
{
    ...
    if ([EHUtils isEmptyString:_customRelationTextFiled.text] && [EHUtils isEmptyString:_selectedRelation])
    {
        //[WeAppToast toast:(@"请选择您与宝贝的关系!")];
        _selectedRelation = @"家人";
    }
    
    if (![EHUtils isEmptyString:_customRelationTextFiled.text]) {
        _selectedRelation = _customRelationTextFiled.text;
    }
    
    if ([self.selectedRelationdelegate respondsToSelector:@selector(selectedRelation:)])
    {
        [self.selectedRelationdelegate selectedRelation:_selectedRelation];
    }
...
- (void)relationTextFieldTextChanged:(NSNotification *)notification
{
    ...
    NSUInteger index = [_relationArray indexOfObject:textField.text];
    ...
    {
        UIView* selectedImageView = [self.view viewWithTag:index+100];
        [_selectedImageView removeFromSuperview];
        
        _selectedImageView.center = CGPointMake(selectedImageView.center.x+25, selectedImageView.center.y - 28);
