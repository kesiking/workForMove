...
@interface EHAudioDataCache ()

@property (strong, nonatomic) NSString *diskCachePath;
@property (strong, nonatomic) NSMutableArray *customPaths;
@property (WeAppDispatchQueueSetterSementics, nonatomic) dispatch_queue_t ioQueue;

@end

@implementation EHAudioDataCache{
...
- (id)initWithNamespace:(NSString *)ns {
    if ((self = [super init])) {
        NSString *fullNamespace = [@"com.hackemist.EHAudioDataCache." stringByAppendingString:ns];
        
        // Create IO serial queue
        _ioQueue = dispatch_queue_create("com.hackemist.EHAudioDataCache", DISPATCH_QUEUE_SERIAL);
...
- (void)dealloc {
    ...
    WeAppDispatchQueueRelease(_ioQueue);
}
...
- (NSString *)cachePathForKey:(NSString *)key inPath:(NSString *)path {
    ...
    return [path stringByAppendingFormat:@"-MySound.wav"];
}
...
- (void)queryDiskCacheForKey:(NSString *)key done:(EHAudioDataQueryCompletedBlock)doneBlock {
    ...
    if (!key) {
        doneBlock(nil, EHAudioDataCacheTypeNone);
        return;
    }
    ...
    dispatch_async(self.ioQueue, ^{
        @autoreleasepool {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                doneBlock(diskAudioData, EHAudioDataCacheTypeDisk);
            });
        }
    });
}
