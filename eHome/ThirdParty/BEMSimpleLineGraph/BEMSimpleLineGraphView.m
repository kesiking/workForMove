...
#pragma clang diagnostic pop
    ...
    if (numberOfPoints == 0) {
        ...
        self.noDataLabel.text = noDataText ?: NSLocalizedString(@"暂无运动数据", nil);
#else
...
- (CGFloat)yPositionForDotValue:(CGFloat)dotValue {
    ...
    if (self.minValue == self.maxValue &&self.minValue!=0&& self.autoScaleYAxis == YES) positionOnYAxis = self.frame.size.height/2;
//    else if (self.autoScaleYAxis == YES) positionOnYAxis = ((self.frame.size.height - padding) - ((dotValue - 0) / ((self.maxValue - self.minValue) / (self.frame.size.height - padding)))) + self.XAxisLabelYOffset/2;
    else if (self.autoScaleYAxis == YES) positionOnYAxis = ((self.frame.size.height - padding) - ((dotValue - 0) / ((self.maxValue - self.minValue) / (self.frame.size.height - padding))));
