...
//新加的
- (UIView *)getCellAtIndex:(NSInteger)index{
    UIView *cell = [_cells objectAtIndex:index];
    return cell;
...
//    CGPoint endPoint = CGPointMake(offset.x - _scrollView.frame.origin.x, offset.y - _scrollView.frame.origin.y);
//    CGPoint startPoint = CGPointMake(MAX(0, endPoint.x) + self.bounds.size.width, MAX(0, endPoint.y) + self.bounds.size.height);
    switch (orientation) {
        case PagedFlowViewOrientationHorizontal:{
...
//            NSInteger startIndex = [_cells count]-1;
//            for (int i =0; i < [_cells count]; i++) {
//                if (_pageSize.width * (i +1) > startPoint.x) {
//                    startIndex = i;
//                    break;
    ...
    if (_needsReload) {
        ...
        switch (orientation) {
            case PagedFlowViewOrientationHorizontal://横向
                ...
                //添加默认的contentOffset by姜伟刚
                _scrollView.contentOffset=CGPointMake(_pageSize.width*(_pageCount-1), 0);
