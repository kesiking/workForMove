...
+(void) InstallUncaughtExceptionHandler
{
    ...
    //判断书就是否为空
    BOOL make = [[exceptionSolve sharedManager] observationMessage];
    if (make) {
        NSLog(@"打印数据 上传至服务器");
        [[exceptionSolve sharedManager] sendMessage];
    }
}
...
//    [alert show];
    ...
    EHLogError(@"#####Unhandled exception:message = %@", [NSString stringWithFormat:NSLocalizedString(
                                                                                                      @"You can try to continue but the application may be unstable.\n"
                                                                                                      @"%@\n%@", nil),
                                                          [exception reason],
                                                          [[exception userInfo] objectForKey:UncaughtExceptionHandlerAddressesKey]]);
...
void UncaughtExceptionHandlers (NSException *exception) {
    ...
    [exceptionSolve sharedManager].log_summary = arr;
    [exceptionSolve sharedManager].log_content = reason;
    [exceptionSolve sharedManager].page_name = name;
    
    [[exceptionSolve sharedManager] DeviceAction];
