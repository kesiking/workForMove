...
- (void)strokeChart
{
    ...
    for (NSUInteger lineIndex = 0; lineIndex < self.chartData.count; lineIndex++) {
        ...
        if (chartData.color) {
            chartLine.strokeColor = [[[UIColor lightGrayColor]colorWithAlphaComponent:chartData.alpha] CGColor];
...
//        CGContextMoveToPoint(ctx, _chartMargin+yAxisOffset, yAxisHeight);
//        CGContextAddLineToPoint(ctx, xAxisWidth, yAxisHeight);
//        
...
//        CGContextAddLineToPoint(ctx, 435, 100);
        CGContextStrokePath(ctx);
