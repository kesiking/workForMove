...
- (void)setDate:(NSDate *)date
{
    ...
    //如果日期超出起止日期，那么day按钮不可点击
    NSDateFormatter *myDateFormatter = [[NSDateFormatter alloc] init];
    [myDateFormatter setDateFormat:@"yyyy-MM-dd"];
    NSString *dayStringNow = [myDateFormatter stringFromDate:date];
    NSString *daytodayString = [myDateFormatter stringFromDate:[NSDate date]];
    NSDate *dateNow = [myDateFormatter dateFromString:dayStringNow];
    NSDate *dayToday = [myDateFormatter dateFromString:daytodayString];
    NSDate *babyStartDay = [[[EHBabyListDataCenter sharedCenter] currentBabyUserInfo] babyDeviceStartUserDay];
    if (babyStartDay == nil ) {
        EHLogInfo(@"babyStartDay is nil");
    }else
    {
        NSString *babyStartDayString = [myDateFormatter stringFromDate:babyStartDay];
        NSDate *dayFrom = [myDateFormatter dateFromString:babyStartDayString];
